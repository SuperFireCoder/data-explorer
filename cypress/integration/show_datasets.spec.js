...
describe(specTitle('login and show fitering on the basis of users on datasets'), () => {
    ...
    it('Show all the datasets', () => {
        cy.get('[data-cy="show-all-data-set"]', { timeout: 5000, force: true }).eq(0).should('contain', 'Show all datasets', { force: true }).click({ force: true })
    ...
    it('Show only my datasets', () => {
        cy.get('[data-cy="show-all-data-set"]', { timeout: 5000, force: true }).trigger('mouseover').click({ force: true });
        cy.get('[data-cy="show-datasets"]').contains('Show only my datasets').as('onlymydatasets');
        cy.get('@onlymydatasets', { force: true }).trigger('mouseover').click({ force: true });
        cy.get('body').click(0, 0);
    ...
    it('Show shared datasets', () => {
        cy.get('[data-cy="show-all-data-set"]', { timeout: 5000 }).should('contain', 'Show all datasets', { force: true }).trigger('mouseover').click({ force: true });
        cy.get('[data-cy="show-datasets"]').contains('Show shared datasets').as('shareddatasets');
        cy.get('@shareddatasets', { force: true }).trigger('mouseover').click({ force: true });
        cy.get('body').click(0, 0);
