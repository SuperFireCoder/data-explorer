...
describe(
    specTitle("filtering Month Filter Monthly"),
    () => {
        beforeEach(() => {
        ...
        it("Filter Month Filter Monthly: January", () => {
                ...
                cy.get('*[class^="bp3-text-overflow-ellipsis bp3-fill"]').first().click()
                .get('.bp3-tag-remove').first().click()
                // I select "January" from Month Filter option
                cy.contains('January').click()
                .get('*[class^="bp3-text-overflow-ellipsis bp3-fill"]').first().should('contain', 'January')
                // assert only 1 Time Domain and 1 Month Filter option is chosen
                .get('body').click(10,10)
                cy.get('*[class^="bp3-text-overflow-ellipsis bp3-fill"]').eq(1).should('contain', 'Current/Historic')
        ...
        it("Filter Month Filter Monthly: November", () => {
            ...
            .get('*[class^="bp3-text-overflow-ellipsis bp3-fill"]').first().should('contain', 'November')
            // assert only 1 Time Domain and 1 Month Filter option is chosen
            .get('body').click(10,10)
            cy.get('*[class^="bp3-text-overflow-ellipsis bp3-fill"]').eq(1).should('contain', 'Current/Historic')
            .get('.bp3-tag').its('length').should('eq', 2)
